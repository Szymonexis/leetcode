...
value of h such that the given researcher has published at least h papers that have 
...
function hIndex(citations: number[]): number {
	const checkArr = Array.from({ length: Math.max(...citations) + 1 }).map(
		() => 0
	);
